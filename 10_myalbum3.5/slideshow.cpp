...
SlideShow::SlideShow(QWidget *parent) : QWidget(parent)
{
    ...
    connect(this,&SlideShow::sigSpeedChange,this,[=](){
        _timer->setInterval(_update_time/_speed);
    });
    connect(_timer,&QTimer::timeout,this,[=](){
        ...
        if(_factor>=1){
            if(cnt==count-1){
                emit this->sigCircle();
                this->setTwoPic(vector_pix[cnt],vector_pix[0]);
                cnt=0;
            }
            else{
                emit this->sigSwitchPic();
                if(cnt<0||cnt>=vector_pix.size()){
                    return;
                }
...
void SlideShow::slotRecvSpeed(double speed)
{
    _speed=speed;
}
